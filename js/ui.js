...
window.UI = {
  ...
  applyPrefs() {
    ...
    $("sw-theme").setAttribute("aria-pressed", !!p.dark);
  ...
  bindMisc() {
    ...
    document.addEventListener("keydown", e => {
      ...
      if (e.key === "Enter" && Quiz.active && Quiz.mode === "atlas") Quiz.next();
    });
  },
  ...
  startCountdown() {
    ...
    const upd = () => {
      const ms = t - Date.now();
      if (ms <= 0) { $("cd-days").textContent = "0"; $("cd-clock").textContent = "sınav geçti"; return; }
      const g = Math.floor(ms / 864e5);
      const s = Math.floor(ms / 36e5) % 24, d = Math.floor(ms / 6e4) % 60;
      $("cd-days").textContent = g;
  ...
  toggleActionButtons(on) {
    ["btn-skip","btn-reveal","btn-stop"].forEach(id => $(id).hidden = !on);
    $("btn-skip").textContent = Quiz.mode === "atlas" ? "Sonraki" : "Pas geç";
    $("btn-reveal").hidden = !on || Quiz.mode !== "normal";
  },
  ...
  renderFound(q, found) {
    ...
    $("q-found").innerHTML =
      `<span class="chip">${found.size} / ${n} il bulundu</span>` +
      Array.from(found).map(k => `<span class="chip found">${esc(titleCase(k))}</span>`).join("");
  },
  ...
  renderReverse(q, options, onPick) {
    ...
    $("q-badge").hidden      = !q.isImportant;
    $("q-found").innerHTML   = `<span class="chip">${q.a.join(", ")}</span>`;
  ...
  renderAtlas(q) {
    ...
    $("q-badge").hidden      = !q.isImportant;
    $("q-found").innerHTML   = q.media
      ? `<button class="ghost" id="atlas-media">Görsel / video aç</button>`
      : `<span class="hint">Bu maddeye görsel eklenmemiş (media alanı boş).</span>`;
  ...
  renderCards() {
    ...
    Store.topics.forEach(t => {
      ...
      el.querySelector('[data-a="del"]').onclick = () => {
        ...
        this.renderCards(); this.fillTopicSelect();
      };
      wrap.appendChild(el);
    });
  ...
  bindComposer() {
    ...
    $("cp-add").onclick = () => {
      ...
      Store.addQuestion(topicId, {
        q: metin, a: this.picks.map(p => p.name), isImportant: $("cp-important").checked
      });
      $("cp-question").value = ""; $("cp-important").checked = false;
  ...
  flash(msg) {
    ...
    if (!el) {
      ...
      el.style.cssText = "position:fixed;left:50%;bottom:22px;transform:translateX(-50%);" +
        ...
        "font-size:.85rem;z-index:90;opacity:0;transition:.2s";
      document.body.appendChild(el);
    }
    el.textContent = msg;
    requestAnimationFrame(() => el.style.opacity = ".95");
  ...
  UI.boot().catch(e => {
    ...
    document.getElementById("map_container").innerHTML =
      `<p style="padding:20px;color:var(--bad)"><b>Harita yüklenemedi.</b><br>${e.message}<br>
       <small style="color:var(--ink-soft)">Kontrol listesi:<br>
       1) js/tr-cities-data.js var mı? (veri-donustur.html ile üretiliyor)<br>
       2) vendor/d3.v5.min.js indirildi mi?<br>
       3) Tarayıcı konsolunu aç (F12) — kırmızı satır ne diyor?</small></p>`;
  });
});
